...
public class ImageUtils {
    ...
    public static void mergeAll(String fileName) {
        ...
        mergeHorizontal(folder);
    }


    public static void mergeVertical(File folder) {
        try {
    ...
    public static void mergeHorizontal(File folder) {
        try {
            File[] files = folder.listFiles();
            int width = 1648, height = 2136;
            assert files != null;
            BufferedImage sumImage = new BufferedImage(width * files.length, height, BufferedImage.SCALE_SMOOTH);
            Graphics2D g2d = sumImage.createGraphics();
            g2d.setComposite(AlphaComposite.Src);
            g2d.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
            g2d.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
            ...
            for (File file : files) {
                if (file.getName().endsWith(".jpg")) {
                    ...
                    BufferedImage image = ImageIO.read(jpg);
                    Image originalImage = image.getScaledInstance(width, height, Image.SCALE_DEFAULT);
                    g2d.drawImage(originalImage, width * i, 0, width, height, null);
                    i++;
                }
            }
            g2d.dispose();
